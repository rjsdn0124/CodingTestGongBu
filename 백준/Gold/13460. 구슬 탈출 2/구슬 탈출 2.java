...
public class Main{
	...
	private static boolean isPassedBlueHole = false, isPassedRedHole = false;
		...
		while(!q.isEmpty()){
			...
			for(int i = 0; i < 4; i++){
				...
				if(i != temp[4] && move(i)){
					...
					if(isPassedBlueHole){
						isPassedBlueHole = false;
						continue;
					}else if(isPassedRedHole){
						result = temp[5];
						return;
					}
	...
	private static boolean move(int direction){
		...
		if(direction < 2 && blue[1] == red[1]){
			boolean isRedBig = (blue[0] < red[0] && direction % 2 == 0) || (blue[0] > red[0] && direction % 2 == 1);
			boolean isMetHole = false;
			blue[0] += dx[direction];
			isMoved = arr[blue[1]][blue[0]];
			while(arr[blue[1]][blue[0]]){
				isMetHole |= blue[0] == hole[0];
				// blue 위치 업데이트.
				blue[0] += dx[direction];
			}
			blue[0] -= dx[direction];
			// 만났을 경우
			if(hole[1] == blue[1] && isMetHole){
				isPassedBlueHole = true;
				return true;
			}
			isMetHole = false;
			red[0] += dx[direction];
			...
			while(arr[red[1]][red[0]]){
				isMetHole |= red[0] == hole[0];
				// red 위치 업데이트.
				red[0] += dx[direction];
			}
			red[0] -= dx[direction];
			if(hole[1] == red[1] && isMetHole){
				isPassedRedHole = true;
				return true;
			}
			if(blue[0] == red[0]){
				if(isRedBig){
		...
		}else if(direction >= 2 && blue[0] == red[0]){
			boolean isRedBig = (blue[1] < red[1] && direction % 2 == 0) || (blue[1] > red[1] && direction % 2 == 1);
			boolean isMetHole = false;
			blue[1] += dy[direction];
			isMoved = arr[blue[1]][blue[0]];
			while(arr[blue[1]][blue[0]]){
				isMetHole |= blue[1] == hole[1];
				// blue 위치 업데이트.
				blue[1] += dy[direction];
			}
			blue[1] -= dy[direction];
			// 만났을 경우
			if(hole[0] == blue[0] && isMetHole){
				isPassedBlueHole = true;
				return true;
			}
			isMetHole = false;
			...
			isMoved |= arr[red[1]][red[0]];
			while(arr[red[1]][red[0]]){
				isMetHole |= red[1] == hole[1];
				// red 위치 업데이트.
				red[1] += dy[direction];
			}
			red[1] -= dy[direction];
			if(hole[0] == red[0] && isMetHole){
				isPassedRedHole = true;
				return true;
			}
			if(blue[1] == red[1]){
